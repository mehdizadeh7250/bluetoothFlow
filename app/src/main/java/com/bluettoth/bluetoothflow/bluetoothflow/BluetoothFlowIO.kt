...
class BluetoothFlowIO(val bluetoothSocket: BluetoothSocket?) {
     ...
     */
    private var socketIsClosed= false
    fun socketIsConnected() = socketIsClosed
    ...
    fun readByteArrayStream(
        ...
        bluetoothSocket?.readByteArrayStream()
        if (inputStream == null) {
            throw NullPointerException("inputStream is null. Perhaps bluetoothSocket is also null")
        }
