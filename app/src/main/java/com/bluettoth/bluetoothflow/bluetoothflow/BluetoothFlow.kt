...
class BluetoothFlow  constructor(private val context: Context) {


    companion object {

        @Volatile
        private var INSTANCE: BluetoothFlow? = null

        fun getInstance(context: Context): BluetoothFlow {
            return INSTANCE ?: synchronized(this) {
                INSTANCE ?: BluetoothFlow(context).also { INSTANCE = it }
            }
        }
    }
    ...
    private var bluetoothFlowIO: BluetoothFlowIO? = null
    ...
    /**
     * Get [BluetoothFlowIO] Helper class for simplifying read and write operations from/to {@link BluetoothSocket}.
     *
     * @param socket bluetooth socket
     * @returns BluetoothFlowIO
     */
    fun getIO(bluetoothSocket: BluetoothSocket): BluetoothFlowIO {

        if (bluetoothFlowIO?.bluetoothSocket === bluetoothSocket) {
            return bluetoothFlowIO as BluetoothFlowIO
        }

        currentBluetoothSocket = bluetoothSocket
        bluetoothFlowIO = BluetoothFlowIO(bluetoothSocket)
        return bluetoothFlowIO as BluetoothFlowIO
    }
    fun isConnected() = bluetoothFlowIO?.isConnected()
    /**
     * Helper class for simplifying read and write operations from/to [BluetoothSocket].
     *
     * @returns BluetoothFlowIO or null
     */
    fun getIO(): BluetoothFlowIO? {

        return currentBluetoothSocket?.let { socket ->
            if (bluetoothFlowIO?.bluetoothSocket === socket)
                return@let bluetoothFlowIO
            bluetoothFlowIO = BluetoothFlowIO(socket)
            return@let bluetoothFlowIO
        }
    }
     ...
     */
    @ExperimentalCoroutinesApi
    suspend fun bluetoothState(): Flow<Int> = callbackFlow<Int> {
        val filter = IntentFilter().apply {
    ...
    fun scanMode(): Flow<Int> = callbackFlow<Int> {
        ...
        val receiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent?) {
                trySend(bluetoothAdapter.scanMode)
            }
        }
        context.registerReceiver(receiver, filter)
        awaitClose {
    ...
    fun bluetoothProfile(bluetoothProfile: Int): Flow<ServiceEvent> = callbackFlow<ServiceEvent> {
        ...
        val listener = object : BluetoothProfile.ServiceListener {
            ...
            override fun onServiceDisconnected(profile: Int) {
                trySend(ServiceEvent(ServiceEvent.State.DISCONNECTED, profile, null)).isSuccess
            }
        }
    ...
    fun closeConnections() = bluetoothFlowIO?.closeConnections()
}